...
export function getBadgeClassName(used: boolean, score?: ExerciseScore) {
  ...
  switch (score) {
    ...
    case 'right':
      return `${base} border-green-600 text-black bg-white hover:border-green-800 hover:bg-green-400 hover:text-white`;

    default:
      if (used)
        return `${base} border-gray-600 text-black bg-white hover:border-gray-800 hover:bg-gray-400 hover:text-white`;
      else
        return `${base} border-blue-600 text-black bg-white hover:border-blue-800 hover:bg-blue-400 hover:text-white`;
  }
}
